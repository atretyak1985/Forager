...
- **`internal/sandbox`** — `Runner` interface (`Exec(ctx, command, timeout)`); `Docker` impl shells out to `docker exec` into the long-lived forager-sandbox container, `Local` is test-only. The container and its `/workspace` volume persist across calls, so files written by one tool call are visible to the next; command failure is carried in `Result.ExitCode`/`TimedOut`, Go errors are reserved for infra failures.
